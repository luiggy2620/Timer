...
window.addEventListener('load', () => { /* Espero a que cargue el documento */
    ...
    alarm = new Audio('sound/callme.mp3'); /* Genero la alarma */
});
...
function setTimer() {
    ...
    clock.innerHTML = `<p class="number">${hours > 9 ? hours : ('0' + hours)}</p><span>:</span><p class="number">${minutes > 9 ? minutes : ('0' + minutes)}</p><span>:</span><p class="number">${seconds > 9 ? seconds : ('0' + seconds)}</p>`;
